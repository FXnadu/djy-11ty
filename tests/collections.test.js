...
function createCollectionApi(posts, photos = []) {
  return {
    getFilteredByGlob(glob) {
      if (glob.includes("content/writing/posts")) return [...posts];
      if (glob.includes("content/photos")) return [...photos];
      return [];
    },
...
describe("collections", () => {
  ...
  it("sorts photo year groups by the year field, newest first", () => {
    const eleventyConfig = createEleventyConfig();
    collections.registerCollections(eleventyConfig);
    ...
    const photoYears = eleventyConfig.registeredCollections.photoYears(createCollectionApi([], photos));

    expect(photoYears.map((entry) => entry.data.year)).toEqual([2026, 2025, 2024]);
  });
});
